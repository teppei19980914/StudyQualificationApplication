...
class ConsistencyCard(QFrame):
    """学習の実施率を表示するカードウィジェット.

    Attributes:
        _theme_manager: テーママネージャ.
        _week_label: 今週の実施率ラベル.
        _month_label: 今月の実施率ラベル.
        _overall_label: 全体の実施率ラベル.
        _data: 実施率データ.
    """
    ...
    def set_data(self, data: ConsistencyData) -> None:
        ...
        week_pct = f"{week_rate * 100:.0f}%"
        week_color = self._rate_color(week_rate, colors)
        self._week_label.setText(
            f"\u4eca\u9031: {week_pct}"
            f"\uff08{data.this_week_days}/{data.this_week_total}\u65e5\uff09"
        )
        ...
        month_pct = f"{month_rate * 100:.0f}%"
        month_color = self._rate_color(month_rate, colors)
        self._month_label.setText(
            f"\u4eca\u6708: {month_pct}"
            f"\uff08{data.this_month_days}/{data.this_month_total}\u65e5\uff09"
        )
    ...
    def _show_empty(self) -> None:
        """初期表示（空状態）."""
        self._week_label.setText("\u4eca\u9031: 0%\uff080/0\u65e5\uff09")
        self._month_label.setText("\u4eca\u6708: 0%\uff080/0\u65e5\uff09")
